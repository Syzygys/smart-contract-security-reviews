...
| [OpenZeppelin Math](reviews/openzeppelin-math.md) | General-purpose integer math — full-precision mulDiv, sqrt, invMod, modExp (Solidity asm) | Static + dynamic (Python cross-check vs. exact math, 4k random cases/fn; worst-case Fibonacci stress test on invMod's unchecked int256 arithmetic) | No vulnerability; mulDiv/sqrt/log2/log10/log256 verified with zero mismatches incl. overflow boundary; invMod's unchecked wraparound verified safe at the exact int256 edge (255/255 bits); found 1 cosmetic comment error; flagged modExp/invModPrime silent-zero on non-EIP-198 chains |
| [Solmate Owned](reviews/solmate-owned.md) | Single-owner access-control mixin (Solidity) | Static, line-by-line + cross-check vs. OpenZeppelin Ownable | No vulnerability; `msg.sender == owner` check correct on every path; flagged (verified vs. OZ source) missing zero-address guard on constructor + transferOwnership and no two-step ownership handshake |
| [Solmate ReentrancyGuard](reviews/solmate-reentrancyguard.md) | Reentrancy-protection base contract (Solidity) | Static, line-by-line + cross-check vs. OpenZeppelin ReentrancyGuard | No vulnerability; non-zero sentinel lock verified correct and atomic; flagged (verified vs. OZ source) `virtual` modifier allows a subclass override to silently disable the guard, plus non-namespaced storage slot vs. OZ's ERC-7201 slot |
| [Solmate WETH](reviews/solmate-weth.md) | Wrapped-Ether custodian contract (Solidity) | Static, line-by-line + cross-check vs. pinned SafeTransferLib/ERC20 source | No vulnerability; burn-before-payout CEI ordering and checked-subtraction solvency check verified; read-only reentrancy window closed by ETH-value/accounting atomicity; flagged `virtual`-override footgun + permanently-unclaimable force-fed ETH |
| [Uniswap V3 FullMath](reviews/uniswap-v3-fullmath.md) | 512-bit full-precision mulDiv/mulDivRoundingUp (Solidity asm, pre-0.8.0) | Static + dynamic (EVM-faithful Python re-implementation vs. exact math, ~28k random/edge trials incl. overflow boundary) | No vulnerability; CRT product, exact-division remainder subtraction, and Newton–Raphson modular inverse all verified with zero mismatches; ceiling-guard confirmed load-bearing under the pre-0.8.0 pragma; flagged pragma-lock vendoring risk + floor-vs-ceiling caller responsibility |
